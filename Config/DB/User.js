...
class UserDB {
	...
	GetEmail() { 
		// this should return all the data in ArtistsDB.
		return new Promise((resolve, reject) => {
			AsyncStorage.getItem(emailName).then(success => {
	...
	GetTicket() {
		// this should return all the data in ArtistsDB.
		return new Promise((resolve, reject) => {
			AsyncStorage.getItem(ticketName).then(success => {
				return resolve(JSON.parse(success));
			}, err => {
				return reject({ error: "Error while fetching the info data from the DB" });
			})
	...
	GetLoggedIn() {
		// this should return all the data in ArtistsDB.
		return new Promise((resolve, reject) => {
			AsyncStorage.getItem(loginInName).then(success => {
				return resolve(success);
			}, err => {
				return reject({ error: "Error while fetching the info data from the DB" });
			})
	...
	SetLoggedIn(num) { 
		...
		return new Promise((resolve, reject) => {
			AsyncStorage.setItem(loginInName, num.toString()).then(success => {
	...
	SetEmail(email) { 
		// set / Replace the InfoContent.
		return new Promise((resolve, reject) => {
			AsyncStorage.setItem(emailName, email).then(success => {
				return resolve({ success: true });
			}, err => {
				return reject({ error: "Error while Save the info data from the DB" });
			})
	...
	SetTicket(InfoContent) { 
		// set / Replace the InfoContent.
		return new Promise((resolve, reject) => {
			AsyncStorage.setItem(ticketName, JSON.stringify(InfoContent)).then(success => {
				return resolve({ success: true });
			}, err => {
				return reject({ error: "Error while Save the info data from the DB" });
			})
